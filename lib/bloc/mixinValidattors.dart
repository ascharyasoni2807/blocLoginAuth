  ...
  var nameValidator = StreamTransformer<String, String>.fromHandlers(
    handleData: (name, sink) {
      if (name.isEmpty && name.length < 3 && name.length > 32) {
        return sink
            .addError("Name length should be greater than 6 less than 32 char");
      }
  ...
  var phoneValidator = StreamTransformer<String, String>.fromHandlers(
    handleData: (phone, sink) {
      if (phone.isEmpty && !(phone.length == 10)) {
        return sink.addError("Inavlid Phone Number");
      }
  ...
  var passwordValidator = StreamTransformer<String, String>.fromHandlers(
    handleData: (password, sink) {
      if (password.isEmpty && password.length < 8) {
        return sink.addError("Password length is less than 8 char");
      }
      if (!RegExp(r"^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[^\da-zA-Z]).{8,15}$")
          .hasMatch(password)) {
        return sink.addError(
            "Password must contain 8 letter, one small,Captial and a special character");
      } else {
        sink.add(password);
      }
    },
